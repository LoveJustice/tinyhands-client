...
export default class AccountController {
    constructor($q, $scope, $timeout, $uibModal, $state, AccountService, PermissionsSetsService) {
        ...
        this.activeTabIndex = $state.current.data.index; 
        ...
        this.sections = [
            { name: this.tab_1_name , state: 'accounts.list'},
            { name: this.tab_2_name , state: 'accounts.control'},
            { name: this.tab_3_name , state: 'accounts.defaults'}
        ];
    ...
    */
    ...
    activateTab(index){
        if(this.activeTabIndex !== index) {
            let state = this.sections[index].state;
            this.$state.go(state);
