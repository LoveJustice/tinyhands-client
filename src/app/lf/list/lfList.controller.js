...
export default class LfListController {
    constructor(LfListService, SessionService, SpinnerOverlayService, StickyHeader, $state, $stateParams, $uibModal, $timeout,  toastr, constants, moment) {
        'ngInject';
        this.service = LfListService;
        ...
        if (!foundStateParams) {
            let tmp = sessionStorage.getItem('lfList-search');
            if (tmp !== null) {
                this.queryParameters.search = tmp;
            }
            tmp = sessionStorage.getItem('lfList-status');
            if (tmp !== null) {
                this.queryParameters.status = tmp;
            }
            tmp = sessionStorage.getItem('lfList-country_ids');
            if (tmp !== null) {
    ...
    get hasAddPermission() {
        return this.session.checkPermission('LF','ADD',null, null) === true;
    }
    ...
    searchLfs() {
        ...
        sessionStorage.setItem('lfList-search', this.queryParameters.search);
        sessionStorage.setItem('lfList-status', this.queryParameters.status);
        sessionStorage.setItem('lfList-country_ids', this.queryParameters.country_ids);
    ...
    addUrls(lfs) {
        for (let idx=0; idx < lfs.length; idx++) {
            let lf = lfs[idx];
            if (lf.form_name !== null) {
                lf.viewUrl = this.state.href(lfs[idx].form_name, {id:lf.id, stationId:lf.station.id, isViewing:true, incidentId: lf.incident, formName: lfs[idx].form_name});
                lf.editUrl = this.state.href(lfs[idx].form_name, {id:lf.id, stationId:lf.station.id, isViewing:false, incidentId: lf.incident, formName: lfs[idx].form_name});
            }
            lf.relatedUrl = this.state.href('relatedForms', {stationId: lf.station.id, formNumber: lf.lf_number});
        }
    }
    ...
    showPage(pageNumber) {
        this.spinnerOverlayService.show("Searching for LFs...");        
        this.service.getLfList(this.transform(this.queryParameters, pageNumber)).then( (promise) => {
            this.lfs = promise.data.results;
            this.paginate.items = promise.data.count;
            this.paginate.currentPage = pageNumber;
            this.spinnerOverlayService.hide();
            this.addUrls(this.lfs);
        });
    }

    deleteLf(lf, index) {
        if (lf.confirmedDelete) {
            this.service.deleteLf(lf.station.id, lf.id).then(
                () => {
                    this.toastr.success("Successfully Deleted LF!");
                    this.lfs.splice(index, 1);
                },
                () => {
                    this.toastr.error("Unable to Delete LF!");
                }
            );
        }
        else {
            lf.confirmedDelete = true;
        }
    }
